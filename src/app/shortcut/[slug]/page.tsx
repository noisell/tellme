...
export default function ShortCut({ params }: { params: { slug: string } }) {
  ...
  const calculateTimeLeft = (apiTime: string): number => {
    ...
    if (apiDate < tenMinutesAgo) {
      ...
      router.replace(`https://t.me/Tellme_tips_bot?startapp=shortcut${params.slug}`)
      return 0
    }
    console.log('Math.ceil', Math.ceil((nowUTC.getTime() - apiDate.getTime()) / 1000))
  ...
  return statusTask === undefined ? (
    ...
    <main className='w-screen h-screen flex relative bg-[#111111] items-center justify-center'>
      <div className='text-white flex flex-col items-center w-full max-w-[400px] bg-[#141414] rounded-3xl'>
        <img src='/thunder.gif' alt='Изображение карточки' width={'70%'} className='mt-5' />
        <h1 className='font-bold text-3xl text-blue-500 mt-5'>Создание ярлыка</h1>
        <div className='flex flex-col px-5 mt-5'>
          <p className=''>1. Нажмите на 3 точки или "Поделиться" в этом окне.</p>
          <p className='mt-3'>2. Выберите "Добавить на главный экран" или "Создать ярлык" и следуйте инструкциям браузера.</p>
          <p className='mt-3'>3. Все готово! Теперь вы сможете удобно заходить в Tellme через главный экран своего телефона.</p>
        </div>
        <h1 className='text-xl mt-5 text-blue-500 pb-4'>Осталось: {formatTime(timeLeft)}</h1>
      </div>
    </main>
  )
}
