...
interface Props {schedule: ExecutorSchedule[] | [], weekday: number, timezoneOffset: number}
...
export function ScheduleSection(props: Props) {
    ...
    const [scheduleData, setScheduleData] = useState<ScheduleData>(
        oldUser
            ? schedule.reduce((acc, executorSchedule) => {
                const { day_of_week, day_off, start_time, end_time } = executorSchedule;
                acc[day_of_week] = day_off ? undefined : {
                    start: {
                        value: (start_time as string).split(':').slice(0, 2).join(':'),
                        open: false,
                        timeClock: <></>,
                    },
                    end: {
    ...
    const [originalDataRef, setOriginalDataRef] = useState<ScheduleData>(scheduleData);
    const updateTime = (value: any,
                        selectionState: PickerSelectionState | undefined,
                        day: keyof ScheduleData, part: keyof ScheduleDataPart) => {
    ...
    useEffect(() => {
        ...
        const changeStateTimeClock = (day: keyof ScheduleData, part: keyof ScheduleDataPart) => {
                    ...
                    [part]: {
                        ...
                        timeClock: <div>
                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                <TimeClock
                                    key={`${day} ${part} ${Date.now()}`} ampm={false}
                                    sx={{width: 'auto', margin: "0"}}
                                    onChange={
                                        (value, selectionState, _) => {
        ...
        const areValuesEqual = Object.entries(scheduleData).every(([day, value]) => {
            const originalValue = originalDataRef[day as keyof ScheduleData];
            return value?.start.value === originalValue?.start.value &&
        ...
        for (const day of daysOfWeekList) {
            ...
            if (scheduleData[dayKey]?.start.open !== previousDataRef.current[dayKey]?.start.open && scheduleData[dayKey] !== undefined) {
                changeStateTimeClock(dayKey, 'start' as keyof ScheduleDataPart)
            }
            if (scheduleData[dayKey]?.end.open !== previousDataRef.current[dayKey]?.end.open && scheduleData[dayKey] !== undefined) {
    ...
    const saveSchedule = () => {
        for (const day of daysOfWeekList) {
            const dayKey = day as keyof ScheduleData;
            if (scheduleData[dayKey]?.start.value === 'Начало' || scheduleData[dayKey]?.end.value === 'Конец') {
                return error()
            }
        }
        setShowMode(true)
        ...
        for (const day of daysOfWeekList) {
            ...
            const executorSchedule: ExecutorSchedule = {
                ...
                start_time: scheduleDataPart === undefined ? null : scheduleDataPart.start.value,
                end_time: scheduleDataPart === undefined ? null : scheduleDataPart.end.value,
            };
        ...
        <div className="flex flex-col w-full h-auto items-center bg-tg-section-color rounded-3xl mt-3 p-4">
            ...
            <div className="flex w-full items-center justify-between pr-2">
                ...
                <div className="flex items-center gap-2">
                    <span className="flex font-normal text-tg-subtitle-color" onClick={() => {
                        setModalOpen(true)
                    }}><ExclamationCircleOutlined style={{fontSize: "18px"}}/></span>
                    {showMode && <span
                        className={`flex font-normal ${mode === 'edit' ? 'text-tg-accent-color' : 'text-tg-subtitle-color'}`}
                        onClick={() => {
                            if (mode === 'view') {
                                setMode('edit')
                            }
                        ...
                        }}><EditRoundedIcon fontSize='small'/></span>}
                </div>
            </div>
            <div className="flex flex-col w-full px-2" style={{fontSize: '14px'}}>
                {!oldUser ?
                    <p className="mr-auto text-xs text-tg-subtitle-color mt-2">
                        У вас еще не создан график работы, давайте это исправим. Нажимайте на время и выбирайте
                        подходящее.
                    </p>

                : mode === 'edit' &&
                    <p className="mr-auto text-xs text-tg-subtitle-color mt-2">
                        Вы находитесь в режиме редактирования, пожалуйста не забывайте сохранять изменения.
                    </p>
                }
                <div className="flex flex-col w-full mt-2">
                    {Object.entries(scheduleData).map(([day, value]) => (
                        <div className="flex w-full items-center justify-between">
                            <span style={{color: 'var(--tg-theme-text-color)'}}>{daysOfWeekMap[day as keyof ScheduleData]}</span>
                            {
                                <span style={{color: 'var(--tg-theme-subtitle-text-color)'}}>
                                    {value ?
                                        (
                                            <span style={{color: 'var(--tg-theme-subtitle-text-color)'}}>
                                                {mode === 'edit' ? <Popover content={value.start.timeClock} trigger="click"
                                                          placement="top"
                                                          open={value.start.open}
                                                          onOpenChange={(open) => setScheduleData(prevState => ({
                                                              ...prevState,
                                                              [day]: {
                                                                  ...prevState[day as keyof ScheduleData],
                                                                  start: {
                                                                      ...prevState[day as keyof ScheduleData]?.start,
                                                                      open: open
                                                                  }
                                                              }
                                                          }))}>
                                                    <button
                                                        style={value.start.open ? {
                                                            textDecoration: "underline 1px var(--tg-theme-accent-text-color)",
                                                            textUnderlineOffset: "3px"
                                                        } : {}}>
                                                        {value.start.value}
                                                    </button>
                                                </Popover> : value.start.value}
                                                <span> - </span>
                                                {mode === 'edit' ? <Popover content={value.end.timeClock} trigger="click" placement="top"
                                                          open={value.end.open}
                                                          onOpenChange={(open) => setScheduleData(prevState => ({
                                                              ...prevState,
                                                              [day]: {
                                                    ...
                                                    <button style={value.end.open ? {
                                                        textDecoration: "underline 1px var(--tg-theme-accent-text-color)",
                                                        textUnderlineOffset: "3px"
                                                    } : {}}>
                                                        {value.end.value}
                                                    </button>
                                                </Popover> : value.end.value}
                                            </span>
                                        )
                                        : (
                                            <span style={{color: 'var(--tg-theme-subtitle-text-color)'}}>Выходной</span>
                                        )
                                    }
                                    {mode === 'edit' && <ConfigProvider theme={{
                                        components: {
                                            Checkbox: {
                                        ...
                                        <Checkbox style={{marginLeft: "5px"}} checked={value !== undefined}
                                                  onChange={(e) => {
                                                      if (e.target.checked) {
                                                          setScheduleData(prevState => ({
                                                              ...prevState,
                ...
                {saveChange && (
                    <div className="flex w-full items-center justify-between mt-3">
                        <button className="p-3 bg-tg-button-color text-tg-text-color rounded-xl"
                                style={{width: "48%"}}
                                onClick={saveSchedule}
                        >
                            Сохранить
                        </button>
                        <button className="p-3 bg-tg-section-second-color text-tg-destructive-text-color rounded-xl"
                                style={{width: "48%"}}
                                onClick={() => {
                                    setScheduleData(originalDataRef)
                                    if (oldUser) {
