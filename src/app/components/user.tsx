...
const colorFind: Record<number, string> = {
  1: 'rgb(234 179 8)',
  2: 'rgb(59 130 246)',
  3: 'rgb(34 197 94)',
  4: 'rgb(168 85 247)',
      ...
      if (r) {
        setLevelsData(
          r.map(item => ({
            value: item.id,
            label: (
              <p className='text-[15px]' style={{ color: `${colorFind[item.id]}` }}>
                {item.name}
              </p>
            ),
          })),
  ...
  return (
    <main className='flex w-full flex-col bg-tg-secondary-background-color items-center'>
      <ConfigProvider
        ...
        <div
          className={`flex flex-col w-full h-auto mt-3 items-center bg-tg-section-color rounded-3xl py-4 px-4 ${window.Telegram.WebApp.colorScheme === 'light' && 'shadow-md shadow-gray-400'}`}>
          <div className='w-full flex flex-col'>
            ...
            <button className='flex w-full mt-3 items-center h-auto bg-tg-button-color rounded-2xl p-3 text-tg-button-text-color justify-center font-bold'>
              Начать поиск · {price}₽
            </button>
