...
import {OrderedListOutlined, CaretRightOutlined, ExclamationCircleOutlined} from "@ant-design/icons";
        ...
        setContent(
            <>
                ...
                <span className="flex flex-col text-left mt-5 gap-y-2">
                    <p className="font-bold" style={{fontSize: "16px"}}>Приглашайте своих друзей в Tellme</p>
                    <p className="text-tg-subtitle-color" style={{fontSize: "14px"}}>
                        Пригласите друзей можно по кнопке "Поделиться" или скопировать ссылку и отправить самостоятельно.
                        Когда кто-то зарегистрируется по вашей ссылке, то вам сразу придет уведомление.
                        Отслеживать всех своих рефералов вы можете в профиле.
                    </p>
                </span>
                <div className="flex w-full items-center justify-between mt-4">
                    <button style={{width: "80%"}}
                            onClick={
                        () => {
                            window.Telegram.WebApp.openTelegramLink(`https://t.me/share/url?text=${text}`)
                        }}
                            className="flex p-3 rounded-xl bg-tg-button-color justify-center gap-1 items-center font-bold text-tg-button-text-color">
                        <p className="ml-5">Поделиться</p> <ReplyIcon fontSize="small" sx={{ transform: 'scaleX(-1)' }}/>
                    </button>
                    <button className="bg-tg-button-color rounded-xl p-3 text-tg-button-text-color"
                            onClick={handleCopy}
                    >
    ...
    const modalSub = (): void => {
        setContent(
            <>
                ...
                <span className="flex flex-col text-left mt-5 gap-y-2">
                    <p className="font-bold w-full text-center" style={{fontSize: "16px"}}>Подписывайтесь на наc в Telegram</p>
                </span>
    ...
    const modalLabel = (): void => {
        setContent(
            <>
                ...
                <span className="flex flex-col text-left mt-5 gap-y-2">
                    <p className="font-bold w-full text-center" style={{fontSize: "16px"}}>Создайте ярлык на рабочий стол</p>
                    <p className="text-tg-subtitle-color" style={{fontSize: "14px"}}>
                        Создав ярлык нашего приложения, вы сможете удобно открывать его и никогда не
                        потеряете бота среди своих чатов. От вас потребуется открыть ссылку в браузере и создать в нем ярлык.
                        Затем перейти по созданному ярлыку и если все успешно, то задание вам зачтется автоматически.
                    </p>
                </span>
            ...
            <div
                ...
                <div className="flex w-full items-center justify-between px-2">
                    ...
                    <ExclamationCircleOutlined
                        style={{fontSize: "18px", color: "var(--tg-theme-subtitle-text-color)"}}
                ...
                <ConfigProvider theme={{
                        ...
                        <div className="flex flex-col items-center bg-tg-section-second-color p-4 rounded-xl"
                             style={{width: '50%'}}
                             onClick={modalFriend}
                        >
                            <Progress type="circle" percent={tasks.invite / 2 * 100}
                                      status={tasks.invite >= 2 ? "success": "active"} size={100}
                                      strokeColor={tasks.invite < 2 ? "var(--tg-theme-accent-text-color)": undefined}
                        ...
                        <div className="flex flex-col items-center bg-tg-section-second-color p-4 rounded-xl"
                             ...
                             onClick={modalSub}
                        >
                            <Progress type="circle" percent={tasks.subscription ? 100 : 0}
                                      status={tasks.subscription ? "success": "active"} size={100}
                                      trailColor="var(--tg-theme-section-bg-color)"/>
                        ...
                        <div className="flex flex-col items-center bg-tg-section-second-color p-4 rounded-xl"
                             ...
                             onClick={modalLabel}
                        >
                            <Progress type="circle" percent={tasks.shortcut ? 100 : 0} status={tasks.shortcut ? "success": "active"} size={100}
                                      trailColor="var(--tg-theme-section-bg-color)"/>
                ...
                <Drawer open={modalOpenFriend} setOpen={setModalOpenFriend} content={content}/>
                <Drawer open={modalOpenSub} setOpen={setModalOpenSub} content={content}/>
                <Drawer open={modalOpenLabel} setOpen={setModalOpenLabel} content={content}/>
            </div>
            : (
