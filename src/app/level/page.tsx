...
export default function Page() {
  ...
  const percent =
    (Number(searchParams.get('completed')) / Number(searchParams.get('need'))) *
    100
  // @ts-ignore
  // @ts-ignore
  const colors = {
            ...
            <div key={number}>
              <div className='flex flex-col items-center'>
                <div className='flex items-center w-full min-h-48 h-auto mb-10'>
                  <div
                    className='flex min-h-48 h-auto items-center justify-center w-1/4'
                    onClick={() => {
                      if (currentPage > 1) {
                        // @ts-ignore
                        ref.current?.prev()
                ...
                {Number(searchParams.get('id')) === currentPage ? (
                  <>
                    ...
                    <span className='text-tg-button-text-color font-bold bg-yellow-500 rounded-2xl py-2 px-10 mt-2'>
                      От 300₽
                    </span>
                  </>
                ) : (
                  <>
                    {levelsData ? (
                      <>
                        ...
                        <p className='text-tg-subtitle-color px-5 w-full text-center text-lg'>
                          Более {getLevelById(currentPage)?.count_orders_start}{' '}
                          советов
                        </p>
                        {currentPage === 2 ? (
                          <span className='text-tg-button-text-color font-bold bg-blue-600 rounded-2xl py-2 px-10 mt-2'>
                            От 500₽
                          </span>
                        ) : currentPage == 3 ? (
                          <span className='text-tg-button-text-color font-bold bg-green-600 rounded-2xl py-2 px-10 mt-2'>
                            От 700₽
                          </span>
                        ) : (
                          <span className='text-tg-button-text-color font-bold bg-purple-600 rounded-2xl py-2 px-10 mt-2'>
                            От 1000₽
                          </span>
                        )}
                      </>
            ...
            <div
              ...
              <div className='flex items-center w-auto h-auto'>
                ...
                <div className='flex flex-col items-start ml-2'>
                  <span>
                    {item.user_name[0].toUpperCase() +
                      item.user_name.substring(1)}
                  </span>
                  <span className='text-xs text-tg-subtitle-color'>
                    Заказов: {item.count}
                  </span>
                </div>
              </div>
