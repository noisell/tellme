...
export function Statistics(props: Props) {
    ...
    const percent_arrow = props.ratio === "up" ? <ArrowUpOutlined/> : <ArrowDownOutlined/>
    ...
    return (
        <div className="flex items-center w-full mt-3 rounded-xl p-3 bg-tg-section-second-color">
            <div className="flex text-3xl ml-1 bg-tg-section-color p-2 rounded-xl" style={{color: arrowColor}}>{arrow}</div>
            <div className="flex flex-col w-full ml-4">
                <span className="text-tg-subtitle-color" style={{fontSize: "14px"}}>{props.title}</span>
                <div className="flex items-end w-full">
                    <span className="text-xl">{value} {props.prefix}</span>
                    <span className="ml-auto" style={{color: arrowColor}}>{percent_arrow} {finalPercent}%</span>
                </div>
            </div>
        </div>
    )
