...
import { authorization, updateShortcutClicks, userInfoForPage } from '@/app/API'
...
export default function Home() {
  ...
  useEffect(() => {
    if (webApp && user) {
      ...
      if (params.has('inline') || params.has('url') || (start_param && (start_param.includes('referrer') || start_param.includes('shortcut')))) {
        const start = async () => {
  ...
  function executorPage(pageData: ExecutorResponseData) {
    ...
    return (
      <main className='flex w-full flex-col bg-tg-secondary-background-color items-center'>
        ...
        <ScheduleSection schedule={executor.executor_schedule} timezoneOffset={user.time_zone} weekday={executor.weekday} />
        <div className='flex w-full h-auto items-center justify-between bg-tg-section-color rounded-3xl mt-3 py-4 px-6 font-medium '>
