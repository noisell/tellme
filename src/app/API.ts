...
import { Categories, ExecuterData, ExecutorSchedule, Level, PageState, Top, UserData, UserTasksCompleted } from '@/app/types'
      ...
      if (error) {
        ...
        if (checkToken) {
          const fetchData = async () => {
            try {
              const response = await axios.post(`${baseURL}/auth/validateJWTToken`, {}, { headers: { Authorization: `Bearer ${value}` } })
...
export async function updateTask(user_id: number, task: 'invite' | 'shortcut' | 'subscription', value: boolean | number) {
  try {
...
export async function updateShortcutClicks(user_id: number, click: number) {
  try {
    await axios.patch(`${baseURL}/user/update/shortcut/time`, { user_id: user_id, count_clicks: click })
  } catch (error: any) {
    return undefined
  }
}
